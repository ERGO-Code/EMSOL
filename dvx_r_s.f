...
CM      ENDIF
CM      IF (sed .EQ. 1) THEN
C?      double precision mx_ed_wt_er
...
CM      IF (dvx .EQ. 1) THEN
      if (ed_wt_o_vr_t_en_bs .gt. one) then
         ed_wt_o_vr_t_en_bs = sqrt(ed_wt_o_vr_t_en_bs)
      else
         ed_wt_o_vr_t_en_bs = one
      endif
      dvx_rao = max(
     &     ed_wt_o_vr_t_en_bs/og_ed_wt_o_vr_t_en_bs,
     &     og_ed_wt_o_vr_t_en_bs/ed_wt_o_vr_t_en_bs)
      i_te = n_r/nw_dvx_fwk_fq
      i_te = max(mn_n_dvx_it, i_te)
      nw_dvx_fwk = dvx_rao .gt. tl_dvx_wt .or. n_dvx_it .gt. i_te
CM      IF (emsol_dev .EQ. 1) THEN
...
CM      ENDIF
CM      IF (sed .EQ. 1) THEN
C?      ed_wt_o_vr_t_en_bs = ed_wt_o_vr_t_en_bs*half
C?      ed_wt_er = abs((
C?     &     og_ed_wt_o_vr_t_en_bs-ed_wt_o_vr_t_en_bs)/
C?     &     og_ed_wt_o_vr_t_en_bs)
C?      if (ed_wt_er .gt. mx_ed_wt_er) then
C?         mx_ed_wt_er = ed_wt_er
C?         if (iand(rsmi_msg_msk, rsmi_er_li_bt) .ne. 0) then
C?            if (ems_msg_no_prt_fm .ge. 1) write(ems_li, 9500)
C?     &           n_si_it, 'Stpst edge wt:',
C?     &           ' U_STPST_ED_WT', og_ed_wt_o_vr_t_en_bs,
C?     &           '         FTRAN', ed_wt_o_vr_t_en_bs, ed_wt_er
...
CM      IF (dvx .EQ. 1) THEN
     ...
     &     call ems_ck_ze_rl_a(n_r, pv_c_v)
CM      ENDIF
CM      IF (emsol_tt .EQ. 1) THEN
C?      if (ems_tt_cz_r_lvl0) call ems_tt_rec(-cz_r_tt, n_bs)
CM      ENDIF
...
CM      IF (sed .EQ. 1) THEN
C? 9500 format('Iteration ', i7, 1x, a14,
C?     &     2(' From ', a14, ' = ', g11.4), ' |Rel. error| = ', g11.4)
CM      ENDIF
...
c
CM      IF (sps_cz_r .EQ. 1) THEN
      subroutine ems_dvx_sps_l1_cz_r(
CM      ELSE
...
CM      IF (dvx .EQ. 1) THEN
     ...
     &     og_ed_wt_o_vr_t_en_bs/ed_wt_o_vr_t_en_bs)
      i_te = n_r/nw_dvx_fwk_fq
      nw_dvx_fwk = dvx_rao .gt. tl_dvx_wt .or.
     &     n_dvx_it .gt. max(mn_n_dvx_it, i_te)
      ed_wt(vr_t_en_bs) = ed_wt_o_vr_t_en_bs
CM      ENDIF
CM      IF (sed .EQ. 1) THEN
C?      ed_wt_o_vr_t_en_bs = ed_wt_o_vr_t_en_bs*half
